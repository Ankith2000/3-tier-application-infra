...
export JWT_SECRET=$(aws secretsmanager get-secret-value --secret-id backend-token-secret | jq -r ".SecretString")
...
export DB_PASSWORD=$(aws secretsmanager get-secret-value --secret-id database-password-secret | jq -r ".SecretString")
